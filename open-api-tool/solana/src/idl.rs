...
impl Display for IdlType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match &self {
            IdlType::Array(a, b) => write!(
                f,
                "[{a}; {}",
                match b {
                    IdlArrayLen::Value(v) => v.to_string(),
                    IdlArrayLen::Generic(g) => g.clone(),
                }
            )?,
            ...
            IdlType::Bytes => write!(f, "[u8]")?,
            IdlType::String => write!(f, "String")?,
            IdlType::Pubkey => write!(f, "pubkey")?,
            IdlType::Option(t) => write!(f, "Option<{t}>")?,
            IdlType::Vec(t) => write!(f, "Vec<{t}>")?,
            IdlType::Defined { name, generics } => write!(
                f,
                "{}{}",
                name,
                &if generics.len() == 0 {
                    ...
                    let generic_str = generics
                        ...
                        .map(|x| match x {
                            ...
                            IdlGenericArg::Type { ty } => format!("{}", ty),
                        })
                        .collect::<Vec<_>>()
                        .join(", ");
                    format!("<{}>", generic_str)
